...
const EVENT_MAPPINGS: UsageEventMapping[] = [
  {
    ...
    metric_type: 'users',
    extractQuantity: (p: unknown) => (p as UserCreatedPayload).quantity ?? 1,
    extractModuleId: (p: unknown) => (p as UserCreatedPayload).module_id,
    extractTenantId: (p: unknown) => (p as UserCreatedPayload).tenant_id,
  },
  {
    event: USAGE_EVENTS.APICallExecuted,
    metric_key: 'api_calls',
    metric_type: 'api_calls',
    extractQuantity: (p: unknown) => (p as APICallExecutedPayload).quantity ?? 1,
    extractModuleId: (p: unknown) => (p as APICallExecutedPayload).module_id,
    extractTenantId: (p: unknown) => (p as APICallExecutedPayload).tenant_id,
  },
  {
    event: USAGE_EVENTS.WorkflowRun,
    metric_key: 'workflow_runs',
    metric_type: 'executions',
    extractQuantity: (p: unknown) => (p as WorkflowRunPayload).quantity ?? 1,
    extractModuleId: (p: unknown) => (p as WorkflowRunPayload).module_id,
    extractTenantId: (p: unknown) => (p as WorkflowRunPayload).tenant_id,
  },
  {
    event: USAGE_EVENTS.StorageUpdated,
    metric_key: 'storage_mb',
    metric_type: 'storage',
    extractQuantity: (p: unknown) => Math.max(0, (p as StorageUpdatedPayload).bytes_delta / (1024 * 1024)),
    extractModuleId: (p: unknown) => (p as StorageUpdatedPayload).module_id,
    extractTenantId: (p: unknown) => (p as StorageUpdatedPayload).tenant_id,
  },
];
