...
export async function checkEmployeeEpiCompliance(
  ...
  const riskAgents = (exposures ?? []).map((e: any) => e.agente_risco);
  ...
  const activeEpiIds = new Set((deliveries ?? []).map((d: any) => d.epi_catalog_id));

  // 4. Check for missing EPIs
  const missingRaw = (mappings ?? []).filter(
    (m: any) => !activeEpiIds.has(m.epi_catalog_id),
  ) as any[];

  // Fetch names for missing
  const missingEpiIds = [...new Set(missingRaw.map((m: any) => m.epi_catalog_id))];
  ...
  if (missingEpiIds.length > 0) {
    const { data: catalogs } = await supabase
      ...
      .select('id, nome')
      .in('id', missingEpiIds);
    epiNames = Object.fromEntries((catalogs ?? []).map((c: any) => [c.id, c.nome]));
  }

  const missing_epis = missingRaw.map((m: any) => ({
  ...
  const now = new Date();
  const expired_epis: ExpiredEpiResult[] = (deliveries ?? [])
    .filter((d: any) => d.data_validade && new Date(d.data_validade) < now)
    .map((d: any) => ({
      delivery_id: d.id,
  ...
  if (deliveryIds.length > 0) {
    ...
    unsigned_deliveries = deliveryIds.filter((id: string) => !signedIds.has(id));
  }
