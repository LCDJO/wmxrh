...
import { Car, Download, MapPin, Play } from 'lucide-react';
import { RouteReplayPanel } from './RouteReplayPanel';

export function TripsTab({ tenantId }: { tenantId: string }) {
  const [trips, setTrips] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [replayTrip, setReplayTrip] = useState<any | null>(null);
  const [replayPositions, setReplayPositions] = useState<any[]>([]);
  ...
  const handleReplay = async (trip: any) => {
    // Load positions for this trip's device & time range
    const { data } = await supabase
      .from('fleet_position_history')
      .select('latitude, longitude, speed, event_timestamp')
      .eq('tenant_id', tenantId)
      .eq('device_id', trip.device_id)
      .gte('event_timestamp', trip.start_time)
      .lte('event_timestamp', trip.end_time)
      .order('event_timestamp', { ascending: true })
      .limit(500);

    if (data && data.length >= 2) {
      setReplayPositions(data);
      setReplayTrip(trip);
    }
  };
  ...
  return (
    <div className="space-y-4">
      {/* Route Replay */}
      {replayTrip && replayPositions.length > 0 && (
        <RouteReplayPanel
          positions={replayPositions}
          deviceName={replayTrip.device_name}
          speedLimitKmh={replayTrip.avg_speed_kmh ? replayTrip.avg_speed_kmh * 1.5 : 80}
          onClose={() => { setReplayTrip(null); setReplayPositions([]); }}
        />
      )}
