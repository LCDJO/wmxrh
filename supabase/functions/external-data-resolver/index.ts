 ...
 *   2. SERPRO Consulta CPF (tenant-configured)
 *   3. IBGE CNAE catalog
...
async function handleSaveCpfConfig(
  ...
  const apiBaseUrl = sanitizeUrl(body.api_base_url || existing.api_base_url || DEFAULT_CPF_CONFIG.api_base_url);
  const endpointPathTemplate = sanitizePathTemplate(
    body.endpoint_path_template || existing.endpoint_path_template || DEFAULT_CPF_CONFIG.endpoint_path_template,
  );
  const isActive = Boolean(body.is_active);

  if (isActive && (!consumerKey || !consumerSecret)) {
    return json({ error: 'Consumer Key e Consumer Secret são obrigatórios para ativar a integração.' }, 400);
  }
  ...
  const configToSave = {
    ...
    docs_url: DEFAULT_CPF_CONFIG.docs_url,
    updated_by: userId,
...
async function handleResolveCpf(
  ...
  const endpointPath = sanitizePathTemplate(config.endpoint_path_template).replace('{cpf}', cpf);
  const requestUrl = `${sanitizeUrl(config.api_base_url)}${endpointPath}`;
...
function normalizeCpfConfig(config: unknown) {
  ...
  return {
    ...
    api_base_url: sanitizeUrl(String(value.api_base_url ?? DEFAULT_CPF_CONFIG.api_base_url)),
    endpoint_path_template: sanitizePathTemplate(String(value.endpoint_path_template ?? DEFAULT_CPF_CONFIG.endpoint_path_template)),
    docs_url: DEFAULT_CPF_CONFIG.docs_url,
  };
}
...
function sanitizeUrl(value: string) {
  return value.trim().replace(/\/$/, '') || DEFAULT_CPF_CONFIG.api_base_url;
}

function sanitizePathTemplate(value: string) {
  const sanitized = value.trim();
  if (!sanitized) return DEFAULT_CPF_CONFIG.endpoint_path_template;
